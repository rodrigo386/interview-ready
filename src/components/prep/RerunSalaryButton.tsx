...
import { rerunSalaryBenchmark } from "@/app/prep/[id]/actions";

export function RerunSalaryButton({
  sessionId,
  isResearching,
  variant = "primary",
}: {
  ...
  return (
    <button
      type="button"
      disabled={disabled}
      onClick={() => startTransition(() => rerunSalaryBenchmark(sessionId))}
      className={className}
    >
      {pending || isResearching ? "Pesquisando…" : "Pesquisar salário"}
    </button>
  );
}
