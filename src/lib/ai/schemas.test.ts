...
/**
 * Salary benchmark — generated by Gemini for the 4th card on Tela 1.
 * Schema is intentionally strict: bad numbers (negatives, absurd ranges,
 * wrong currency) should be rejected so the UI never renders nonsense.
 * sanitizeSalaryBenchmark in gemini.ts handles the common coercions
 * (string amounts, centavos values) BEFORE this schema runs.
 */
describe("salaryBenchmarkSchema", () => {
  const validSalary = {
    ...
    notes: "Faixa pra pleno em consultoria em SP, CLT.",
    confidence: "medium" as const,
  };

  it("accepts the canonical happy-path shape", () => {
    expect(salaryBenchmarkSchema.safeParse(validSalary).success).toBe(true);
  });

  it("rejects negative amounts", () => {
    const r = salaryBenchmarkSchema.safeParse({ ...validSalary, min_brl: -100 });
    expect(r.success).toBe(false);
  });

  it("rejects amounts above the R$ 1M ceiling", () => {
    const r = salaryBenchmarkSchema.safeParse({
      ...validSalary,
      max_brl: 5_000_000,
    });
    expect(r.success).toBe(false);
  ...
  it("rejects non-integer amounts (Gemini sometimes emits floats)", () => {
    const r = salaryBenchmarkSchema.safeParse({
      ...validSalary,
      median_brl: 12000.5,
    });
  ...
  it("rejects unknown currency", () => {
    const r = salaryBenchmarkSchema.safeParse({ ...validSalary, currency: "USD" });
    expect(r.success).toBe(false);
  });

  it("rejects unknown seniority enum value", () => {
    const r = salaryBenchmarkSchema.safeParse({
      ...validSalary,
      seniority: "diretor-supremo",
    });
    expect(r.success).toBe(false);
  });

  it("makes employment_type_hint optional", () => {
    const { employment_type_hint: _unused, ...withoutHint } = validSalary;
    expect(salaryBenchmarkSchema.safeParse(withoutHint).success).toBe(true);
  });

  it("requires region_hint (forces scope context)", () => {
    const { region_hint: _unused, ...withoutRegion } = validSalary;
    expect(salaryBenchmarkSchema.safeParse(withoutRegion).success).toBe(false);
  });

  it("rejects notes shorter than 10 chars (forces qualitative context)", () => {
    expect(
      salaryBenchmarkSchema.safeParse({ ...validSalary, notes: "curto" }).success,
    ).toBe(false);
  });

  it("rejects notes longer than 500 chars", () => {
    expect(
      salaryBenchmarkSchema.safeParse({ ...validSalary, notes: "x".repeat(501) })
        .success,
    ).toBe(false);
  });

  it("accepts all 7 seniority enum values", () => {
    const all = [
      "estagio",
      "junior",
      "pleno",
    ...
    for (const s of all) {
      expect(
        salaryBenchmarkSchema.safeParse({ ...validSalary, seniority: s }).success,
      ).toBe(true);
    }
  });

  it("accepts all 3 confidence enum values", () => {
    for (const c of ["low", "medium", "high"] as const) {
      expect(
        salaryBenchmarkSchema.safeParse({ ...validSalary, confidence: c })
          .success,
      ).toBe(true);
    }
  });
