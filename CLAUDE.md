...
> ⏸️ **Front-end PAUSADO desde 2026-07-01** ("não vamos utilizar por enquanto"). As rotas públicas `/parceiros` e `/partner` redirecionam pra `/` (307 temporário em `next.config.ts`), o item "Programa de parceiros" saiu do `AvatarMenu`, `/sobre` não menciona mais, e o IndexNow não pinga `/parceiros`. **Backend intacto** (tabelas `affiliate_*`, webhook TRANSFER_*/clawback, `/admin/affiliates`, atribuição `pv_ref` no middleware) — nada foi deletado. **Reativar** = remover os 2 blocos de redirect no `next.config.ts` + restaurar o `<Link href="/parceiros">` no `AvatarMenu`. Commit `b79416a`. A doc abaixo descreve o comportamento quando estava/estiver ligado.
